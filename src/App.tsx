...
export function AppShell({
  ...
  function getAvailableIds(
    ...
    nextSwitchedPairIds: string[] = switchedPairIds,
  ): Set<string> {
    const nextEffectiveCards = filterCardsBySkillSwitch(
      cards,
      skillSwitchPairs,
      new Set(nextSwitchedPairIds),
      nextProducerLevel,
    )
  ...
  function handleEventChange(nextEventId: string) {
    ...
    const nextAvailableIds = getAvailableIds(nextEvent ?? null, producerLevel)
  ...
  function handleProducerLevelChange(nextProducerLevel: number) {
    const nextAvailableIds = getAvailableIds(currentEvent, nextProducerLevel)
  ...
  function handleSkillSwitchChange(pairId: string, mode: SkillSwitchMode) {
    ...
    const nextAvailableIds = getAvailableIds(
      currentEvent,
      producerLevel,
      nextSwitchedPairIds,
    )
